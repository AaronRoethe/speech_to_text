...
def speech_to_text(src, files, batch_name):
    pd.DataFrame(columns=['name','text']).to_csv(batch_name,index=False)
    for file in files:
...
def pipeline(start, rename, convert, load):
    start = create_folder(start)
    files = os.listdir(start)

    step2 = create_folder(rename)
    rename_file(start, files, step2)

    step3 = create_folder(convert)

    subprocess.run(f"FOR %i IN (test_data/named/*.mp4) DO ffmpeg -i test_data/named/%i -acodec pcm_s16le -ar 16000 test_data/audio/%i.wav", shell=True)

    step4 = os.listdir(step3)
    speech_to_text(step3, step4, load)

pipeline('test_data/raw', 'test_data/named', 'test_data/audio', "test_data/load/final.csv")